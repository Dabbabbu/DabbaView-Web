...
- **Crosslink** (PACS 스카우트 방식): **선택한 칸** 시리즈의 스캔 플래닝을 **다른 칸 위에** — 전체 슬라이스는 **파란 점선**, 지금 슬라이스는 **노란 실선**. 선택한 칸에는 선이 없고, 다른 칸을 누르면 그 칸이 기준이 됩니다
...
- **개인정보처리방침**: [privacy.html](https://dabbabbu.github.io/DabbaView-Web/privacy.html) (한국어 · English) — Play 스토어 등록에도 이 주소를 씁니다.
- **안드로이드 앱(TWA)**: 같은 웹앱을 담은 Play 스토어용 앱을 Bubblewrap으로 만들 수 있습니다 (패키지 `com.dabbabbu.dabbaview`). 앱은 이 사이트를 그대로 열기 때문에 웹을 배포하면 앱도 최신이 됩니다.
...
### 어디서 불러왔는지 · 여러 폴더 함께
- 시리즈 카드 아래 `📁 폴더 이름`(압축파일이면 `묶음.zip ▸ 안쪽 폴더`, 클라우드는 드라이브 폴더 이름), 영상 왼쪽 아래 `📄 파일 이름`, 브라우저 탭 제목에 폴더 이름
- **열기 ▸ 파일 열기 · 폴더 열기**는 지금 목록을 새것으로 바꾸고, **➕ 파일 추가 · ➕ 폴더 추가**는 지금 목록에 **더해** 빈 칸에 **나란히** 놓습니다 (빈 칸이 없으면 격자가 1×1 → 1×2 → 2×2로 커짐). 끌어다 놓기는 늘 더하기입니다
- **Crosslink · Ref Line 연동 기준**(기능 찾기 ⌘F → "연동 기준"): 좌표계가 같을 때만(기본) / 같은 검사면 / 환자 좌표만 보고 항상
- OneDrive 창: 🔄 새로 고침(새로 생긴 항목 🆕 표시), 항목 앞 체크 상자로 여러 개 고르기

### 압축파일
- **ZIP · 7z · RAR · TAR(.tar/.tgz/.tar.gz/.tar.bz2/.tar.xz) · ISO · .gz**를 열기 · 끌어다 놓기 · 클라우드로 열면 브라우저 안에서 풀어서 엽니다 (서버로 올리지 않음). 압축 안의 압축(두 단계), 윈도우 ZIP의 한글 이름도 됩니다.
- ZIP · TAR · gz는 브라우저 내장 기능, 7z · RAR · ISO · bz2 · xz · ZIP64는 [libarchive.js](https://github.com/nika-begiashvili/libarchivejs)(WebAssembly, 처음 한 번 약 1 MB)로 풉니다.
- 제한: 풀린 파일은 모두 메모리에 올라가므로 **2 GB가 넘는 압축파일은 풀지 않습니다**(폰 · 태블릿은 더 작은 파일도 느리거나 실패할 수 있음). 암호가 걸린 압축파일은 풀 수 없습니다.
...
| **영상 아래 🔍 Zoom / ◐ W/L 칸** 누르고 끌기 | 영상을 가리지 않고 확대(위아래) · W/L(좌우 = Width, 위아래 = Level). 두 번 클릭: 화면 맞춤 / 기본값 · 기능 찾기에서 숨기기 |
...
버전은 `package.json`의 `version` 하나만 기준으로 삼습니다. 빌드할 때 `vite.config.js`가 이 값을 `__APP_VERSION__`으로 코드에 넣고(`src/version.js`), 화면에는 **시리즈 패널 아래**, **시작 화면**, **ⓘ 정보 창**에 `DabbaView Web v4.0.0` 형태로 표시됩니다. 4.0.0부터 데스크톱 DabbaView와 같은 주 버전 번호를 씁니다.
